...
import { AddToWatchlistResponseType, MovieDataType } from "apps/WatchList/types/WatchListTypes";
// helper
...
export const extendedApiSlice = apiSlice.injectEndpoints({
    endpoints: (builder) => ({
        ...
        addMovieToWatchList: builder.mutation<AddToWatchlistResponseType, MovieDataType>({
            query: (movieData) => {
        ...
        deleteMovieFromWatchList: builder.mutation<any, { movieId: string | number }>({
            query: ({ movieId }) => ({
                url: `/posts/${movieId}`,
            ...
            onQueryStarted({ movieId }, { dispatch, queryFulfilled }) {
                const finalWatchlist = dispatch(
                    extendedApiSlice.util.updateQueryData("getWatchList", undefined, (data) => {
